...
pub struct ImpulseCache(HashMap<[ecs::IdType; 2], f32>);
...
impl ImpulseCache {
    ...
    pub(self) fn get(&self, ids: [ecs::IdType; 2]) -> Option<&f32> {
        if ids[0] < ids[1] {
            self.0.get(&ids)
        } else {
...
impl<'a, I> System<'a> for ConstraintSolver<'a, I>
where
    I: phys::Integrator,
{
    ...
    fn run_system(
        self,
        items: &mut [Self::Query],
        _space: &ecs::Space,
        _queue: &mut ecs::event::EventQueue,
    ) {
        ...
        let id_index_map: HashMap<ecs::IdType, usize> = items
            .iter()
            .enumerate()
